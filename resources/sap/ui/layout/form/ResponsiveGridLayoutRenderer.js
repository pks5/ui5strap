...
sap.ui.layout.form.ResponsiveGridLayoutRenderer.renderContainers=function(r,l,f){var c=f.getFormContainers();var v=[];var L=0;for(var i=0;i<c.length;i++){var C=c[i];if(C.getVisible()){L++;v.push(C)}}if(L>0){if(L>1){r.renderControl(l._mainGrid)}else if(l.mContainers[v[0].getId()][0]){r.renderControl(l.mContainers[v[0].getId()][0])}else{r.renderControl(l.mContainers[v[0].getId()][1])}}};
