...
sap.ui.define(['jquery.sap.global', 'sap/ui/model/TreeBinding', 'sap/ui/model/odata/CountMode'],
	...
	*
	* @class
	* Tree binding implementation for client models. The ODataTreeBinding does only support CountMode.Inline. 
	* This CountMode is set as default.
	*
...
//			}
	...
	ODataTreeBinding.prototype._loadSubNodes = function(sNodeId, iStartIndex, iLength, iThreshold, aParams, mParameters) {
				...
				if (!that.bHasTreeAnnotations) {
					...
					for (var i = 0; i < oData.results.length; i++) {
						...
						if (!(sNodeId in that.oKeys)) {
							...
							that.oLengths[sNodeId] = iResultLength;
							that.oFinalLengths[sNodeId] = false;
		...
		if (sNodeId) {
			if (!this.oFinalLengths[sNodeId]) {
				...
				this.oRequestHandle = this.oModel.read(sAbsolutePath, {urlParameters: aParams, success: fnSuccess, error:fnError});
			}
		}
	};
	...
	ODataTreeBinding.prototype._hasTreeAnnotations = function() {
		...
		if (bMissing && !this.oNavigationPaths) {
			jQuery.sap.log.error("A navigation paths parameter object has to be defined");
			this.oNavigationPaths = {};
		}
