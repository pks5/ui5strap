...
/*
	...
	*/
	NavContainerBaseProto.toPage = function(page, target, transitionName, callback){
		...
		if(!transitionName){
			changeTransitionName = this.getDefaultTransition();
		}
...
(function(){
	...
	var _preloadLibraries = function(_this, libs, callback){
		...
		for(var i = 0; i < libs.length; i++){
			...
			if(libPackage in _KnownLibraryIssues){
				...
				jQuery.sap.log.debug("Fix for library '" + libPackage + "' loaded.");
			}
	...
	*/
	...
	ViewerMultiProto.hideApp = function(transitionName, callback){ 
		this.showApp(HOME_ID, transitionName, function showAppComplete(appInstance){
			callback && callback();
			//appInstance.hidden();
		});
	};
...
(function(){
	...
	*/
	AMCloseOverlayProto.run = function(){
		var _this = this;
		this.context.app.hideOverlay(function CloseOverlayActionComplete(){
			_this.fireEvents(ui5strap.ActionModule.EVENT_COMPLETED);
...
(function(){
	...
	*/
	AMShowOverlayProto.run = function(){
		...
		app.showOverlay(viewOptions, function AMShowOverlayRunComplete(){
			_this.fireEvents(ui5strap.ActionModule.EVENT_COMPLETED);
		}, this.getParameter('overlayTransition'));
	};
...
(function(){
  ...
  CarouselProto.setIndex = function(newIndex){
    ...
    if(!this.getDomRef()){ 
      ...
      this.$().attr("class", this._cssClasses());
