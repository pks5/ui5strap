	...
	ActionModuleProto.findControl = function(){
		...
		if("APP" === scope){
			...
			if(!this.context.control){
				throw new Error("Cannot use scope 'CONTROL': no 'control' in context!");
			}

			theControl = this.context.control;
		}
		else if("SOURCE" === scope){
			//@dperecated
			if(!this.context.control){
				throw new Error("Cannot use scope 'SOURCE': no 'control' in context!");
			}
			
			jQuery.sap.log.warning('Scope SOURCE is deprecated for actions. Use CONTROL instead.');

			theControl = this.context.control;
		}
		else if("SELECTION" === scope){
			if(!this.context.control || !this.context.control.getSelectedControl){
				throw new Error("Cannot use scope 'SELECTION': no 'control' in context or no selection support!");
			}

			theControl = this.context.control.getSelectedControl();
		}
	...
	*/
	var _extendContextFromEventsAndMerge = function(context){
		...
		if("oEvent" in context){ 
			...
			context.channel = context.oEvent.sId;
...
(function(){
	...
	*/
	...
	var _createActionEventHandler = function(controllerImpl, eventName){
		...
		controllerImpl[eventFunctionName] = function(orgEvent){ 
			var app = this.getApp();
			...
			if(oldOnPageShow){
				oldOnPageShow.call(this, orgEvent);
			}
		};
	};
		...
		if(compConfig.events){
			...
			for(var i = 0; i < compConfig.events.length; i++){
				...
				if(stringParts.length === 2){
					//Register app event
					this.registerEventAction(stringParts[0], stringParts[1], function on_event(orgEvent){
