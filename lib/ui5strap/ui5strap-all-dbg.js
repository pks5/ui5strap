	...
	*/
	ui5os.init = function(options, callback){
		...
		var onDeviceReady = function(){
			...
			var appConfig = new ui5strap.AppConfig(options);
...
(function (){
	...
	*/
	...
	AppConfigProto.resolve = function(appInstance){
		var configDataJSON = this.data,
			viewerOptions = this.options,
			appId = appInstance.getId();
		...
		for(var i = 0; i < iconKeysLength; i++){
			configDataJSON.iconsResolved[iconKeys[i]] = appInstance.resolvePath(configDataJSON.icons[iconKeys[i]]);
		}
...
(function(){
	...
	*/
	var _preloadJavaScript = function(_this, callback){
		...
		for(var i = 0; i < scripts.length; i++){
			var jsPath = _this.resolvePath(scripts[i]);
	...
	*/
	...
	AppProto.preload = function(callback){
		...
		_preloadModels(_this, _configData.models, appBase, function(){
			...
			_this.getRootControl().placeAt(_this.config.options.container || 'ui5strap-body');
	...
	*/
	AppProto.initFrame = function(){
		...
		this.config.data.components.unshift(frameConfig);
	};
	...
	AppProto.includeStyle = function(callback){
		...
		for(var i = 0; i < callbackCount; i++){
			var cssKey = cssKeys[i],
				cssPath = this.resolvePath(configData.css[cssKey]);
	...
	*/
	...
	AppProto.resolvePath = function (path){
		var location = this.config.data.app.location;

		if(typeof path === 'string'){
			if(jQuery.sap.startsWith(path, '/')){
				//Root path to the level of the servlet root directory
				return this.config.options.pathToServletRoot + path;
			}
 ...
 * UI5Strap
 *
 * Standard NavContainer with 3 targets "navbar", "sidebar" and "content"
 * 
 * @author Jan Philipp Knöller <info@pksoftware.de>
 * 
 * Homepage: http://ui5strap.com
