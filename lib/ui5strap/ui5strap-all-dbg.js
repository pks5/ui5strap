...
(function(){
  ...
  */
  ui5strap.controller = function(controllerName, controllerImpl){
      jQuery.sap.require('ui5strap.App');

      ui5strap.App.blessController(controllerImpl);
	...
	*/
	var _init = function(_this, action){
		...
		if(action.event){ //Expected sap.ui.base.Event instance 
			...
			if(eventSource){ //Expected sap.ui.base.EventProvider
...
(function(){
	...
	var _showApp = function(_this, appInstance){
		...
		appInstance.onShow(new sap.ui.base.Event("app.show", _this, {}));

		if(!appInstance.hasFirstShow){
			appInstance.hasFirstShow = true;
			appInstance.onFirstShow(new sap.ui.base.Event("app.firstShow", _this, {}));
		}
	};
...
(function(){
	...
	* @Static
	*/
	App.blessController = function(controllerImpl){
	...
	*/
	AppProto.init = function(){
		this.onInit(new sap.ui.base.Event("app.init", this, {}));
	};
	...
	*/
	AppProto.load = function(callback){
		...
		_preloadComponents(_this, function _preloadComponents_complete(){
			_preloadModels(_this, _configData.models, appBase, function _preloadModels_complete(){
				...
				_this.includeStyle(function includeStyle_complete(){
					_preloadJavaScript(_this, function preloadJavaScript_complete(){
						_preloadViews(_configData.views, function preloadViews_complete(){
							_this.onLoad(new sap.ui.base.Event("app.load", _this, {}));
	...
	AppProto.start = function(){
		...
		window.addEventListener(
			...
			function on_message(event){
				_this.onMessage(new sap.ui.base.Event("message", _this, event.data));
			}, 
			false
		);

		this.onStart(new sap.ui.base.Event("app.start", _this, {}));
	};
	...
	*/
	AppProto.stop = function(){
		...
		this.onHide(new sap.ui.base.Event("app.hide", this, {}));
		
		this.onHidden(new sap.ui.base.Event("app.hidden", this, {}));

		this.onStop(new sap.ui.base.Event("app.stop", this, {}));
	};
	...
	*/
	...
	AppProto.getRootControl = function(){
		var control = this.getFrame().control;
		if(!control){
			throw new Error('Frame control is null.');
		}
		return control;
	};
	...
	* @public
	*/
	AppProto.onMessage = function(oEvent){
	...
	* @public
	*/
	AppProto.onInit = function(oEvent){
		this.fireEventAction({ 
			"scope" : "app",
			"eventName" : "init",
			"orgEvent" : oEvent
	...
	* @public
	*/
	AppProto.onLoad = function(oEvent){
		this.fireEventAction({ 
			"scope" : "app",
			"eventName" : "load",
			"orgEvent" : oEvent
	...
	* @public
	*/
	AppProto.onUnload = function(oEvent){
		this.fireEventAction({ 
			"scope" : "app",
			"eventName" : "unload",
			"orgEvent" : oEvent
	...
	* @public
	*/
	AppProto.onStart = function(oEvent){
		this.fireEventAction({ 
			"scope" : "app",
			"eventName" : "start",
			"orgEvent" : oEvent
	...
	* @public
	*/
	AppProto.onStop = function(oEvent){
		this.fireEventAction({ 
			"scope" : "app",
			"eventName" : "stop",
			"orgEvent" : oEvent
	...
	* @public
	*/
	AppProto.onShow = function(oEvent){
		this.fireEventAction({ 
			"scope" : "app",
			"eventName" : "show",
			"orgEvent" : oEvent
	...
	* @public
	*/
	AppProto.onShown = function(oEvent){
		jQuery.sap.log.debug('[APP] ON SHOWN');
	...
	* @public
	*/
	AppProto.onFirstShow = function(oEvent){
		var _this = this;

		jQuery.sap.log.debug('[APP] ON FIRST SHOW');

		this.getFrame().showInitialContent(function showInitialContent_complete(){
			_this.setLoaderVisible(false, function setLoaderVisible_complete(){
				_this.setSplashVisible(false, function setSplashVisible_complete(){
					_this.onShown(new sap.ui.base.Event("app.shown", _this, {}));

					if(!_this.hasFirstShown){
						_this.hasFirstShown = true;
						_this.onFirstShown(new sap.ui.base.Event("app.firstShown", _this, {}));
					}
				});
			});
		});
	...
	* @public
	*/
	AppProto.onFirstShown = function(oEvent){
		this.fireEventAction({ 
			"scope" : "app",
	...
	* @public
	*/
	AppProto.onHide = function(oEvent){
		this.fireEventAction({ 
			"scope" : "app",
			"eventName" : "hide",
			"orgEvent" : oEvent
	...
	* @public
	*/
	AppProto.onHidden = function(oEvent){
		this.fireEventAction({ 
			"scope" : "app",
			"eventName" : "hidden",
			"orgEvent" : oEvent
	...
	* Execute an Action
	*/
	AppProto.runAction = function(action){
		action.app = this;
	...
	*/ 
	 ...
	 * Create a new page
	 */
	AppProto.createView = function(viewConfig){
	...
	*/ 
	...
	AppProto.loadComponent = function(componentId, callback){
		...
		if(compConfig.events){
			...
			for(var i = 0; i < compConfig.events.length; i++){
				...
				if(stringParts.length === 2){
					//Register app event
					this.registerEventAction(stringParts[0], stringParts[1], function on_event(oEvent){
						//Executing component's event handler
						var comp = _this[methodName](),
							eventHandlerName = 'on' + jQuery.sap.charToUpperCase(oEvent.getId().split('.')[1]);
						if(eventHandlerName in comp){
							comp[eventHandlerName](oEvent);
						}
					});
				}
			}
...
(function(){
	...
	var _triggerControllerEvent = function(_this, target, oControl, eventId, eventParameters){
		if(oControl){
			...
			if(controller && controller[funcName]){
				...
				controller[funcName](new sap.ui.base.Event(eventId, _this, eventParameters || {}));
			}
		}
	};
	...
	var _Transition = function(transitionName, $currentRoot, $nextRoot, transitionId){
		...
		this.execute = function (currentRootCallback, nextRootCallback){
			...
			if(transitionName && _transitionEndEvent){
				...
				this.$next && this.$next.removeClass('navcontainer-page-hidden');
	...
	*/
	var _prepareTransition = function(pageChange){
		if(pageChange.transition){
			//jQuery.sap.log.debug(' - [NC] T2A {' + pageChange.target + '}');

			return false;
		}
		else{
			//jQuery.sap.log.debug(' + [NC] T2 {' + pageChange.target + '}');
	...
	*/
	var _preparePendingTransitions = function(_this){
		...
		for(var i = 0; i < pendingTransitionsLength; i++){
			var pageChanges = _this._targetTransitions[_this._pendingTransitions[i]];
			successAll = _prepareTransition(pageChanges[pageChanges.length-1]) && successAll;
		}
	...
	*/
	var _handlePendingTransitions = function(_this){
		...
		_requestAnimFrame(function RAF1(){
			...
			_requestAnimFrame(function RAF2(){
	...
	*/
	NavContainerBaseProto.toPage = function(page, target, transitionName, callback){
		...
		var changeTransitionName = transitionName;
		if(!transitionName){
			changeTransitionName = this.defaultTransition;
		}
		else if(transitionName === 'no-transition'){
