		...
		var complete = function(){
			...
			if(callI === 0){
				...
				if(!_this.initialized){
					...
					_this.onInitialize && _this.onInitialize();
				}
...
(function(){
	...
	*/
	AMLoadModelProto.run = function(){ 
			var modelUrl = this.context.app.resolvePath(this.getParameter("src"));
			...
			this.context._log.debug(this._actionNameShort + "Model '" + modelName + "' (type: '" + modelType + "', scope: '" + this.getParameter("scope") + "') loaded.");
	};
...
(function(){
	...
	NavContainerRenderer.render = function(rm, oControl) {
		...
		for(var aggregationId in aggregations){
			var aggData = aggregations[aggregationId];
			if(!aggData.multiple){
	...
	*/
	NavContainerRenderer.renderAggregation = function (rm, oControl, aggregationName) {
			...
			//Pages container
			rm.write('<div id="' + oControl.targetPagesDomId(aggregationName) + '"');
			rm.addClass('navcontainer-pages');
			rm.writeClasses();
			rm.write("></div>");
			
			//Layers container
			rm.write('<div id="' + oControl.targetLayersDomId(aggregationName) + '"');
