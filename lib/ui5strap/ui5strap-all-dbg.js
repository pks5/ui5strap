...
(function(){
	...
	*/
	ActionContextProto.parameterKey = function (parameterKey, prefix){
		if(!this.parameters.__version || !__versions[this.parameters.__version]){
			throw new Error('Cannot read parameter "' + parameterKey + '": Invalid action version: ' + this.parameters.__version);
		}
	...
	var _loadActionFileStack = function(context, actionName, callback){
		if(actionName){ //Expected string or object
			...
			_loadActionFromFile(context, actionName, function _loadActionFromFile_complete(actionJSON){
				...
				if(context.parameters.__version && context.parameters.__version !== actionJSON.__version){
					throw new Error('Cannot load action "' + actionName + '": Bad version: ' + actionJSON.__version + " (expected: " + context.parameters.__version + ")");
				}
				else{
					context.parameters.__version = actionJSON.__version;
				}
...
(function(){
	...
	*/
	AppProto.setTheme = function(themeName){
		...
		sap.ui.getCore().applyTheme(themeName, this.config.options.pathToServletRoot + '/theme');
