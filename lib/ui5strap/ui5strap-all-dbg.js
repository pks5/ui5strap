	...
	var _init = function(_this, action){
		...
		if(action.parameters){
			...
			if(actionParametersType === 'object'){
				...
				if(!_this.DEFAULT.__version){
					_this.DEFAULT.__version = ActionContext.DEFAULT_FORMAT;
				}
			}
			else{
	...
	var _loadActionFileStack = function(context, actionName, callback){
		if(actionName){ //Expected string or object
			...
			_loadActionFromFile(context, actionName, function _loadActionFromFile_complete(actionJSON){
				...
				//Format check
				var formatOfFile = actionJSON.__format || ActionContext.DEFAULT_FORMAT;
				if(context.parameters.__format && context.parameters.__format !== formatOfFile){
					throw new Error('Cannot load action "' + actionName + '": Bad format: ' + formatOfFile + " (expected: " + context.parameters.__format + ")");
				}
				else{
					//If no format string prensent in context, set it to the format of the JSON
					context.parameters.__format = formatOfFile;
				}
...
(function(){
	...
	AppFrameProto.init = function(){
		...
		if(this.getNavContainer){
			throw new Error("The method 'ui5strap.AppFram.prototype.getNavContainer' has been removed. Please override the method 'ui5strap.AppFram.prototype._initControl', and set the object property 'control' to your NavContainer instance there.");
		}
	};
...
(function(){
	...
	*/
	App.blessController = function(controllerImpl){
        ...
        //@deprecated
		controllerImpl["a_run"] = function(oEvent){
