...
(function(){
	...
	*/
	ActionModuleProto.execute = function(){
		...
		this.context._functions(this.context.parameterKey(ActionContext.PARAM_FUNCTIONS, "parameters." + this.namespace));
		...
		if("APP" === scope){
			...
			if(!controlId){
				if(!this.context.controller){
					throw new Error("Cannot use scope 'VIEW' without 'controlId': no 'controller' in context!");
				}
...
(function(){
	...
	*/
	...
	/*
	* Extend the context by data from Events
	* @private
	* @static
	*/
	var _extendContextFromEventsAndMerge = function(context){
		...
		if("event" in context){ 
			...
			context.channel = context.event.getId();

			//Event Source
			var eventSource = context.event.getSource();
			...
			var eventParameters = context.event.getParameters();
			if(eventParameters){
				context.eventParameters = eventParameters;
			}
		}
	...
	var _extendContextFromFileOrMerge = function(context, actionName, callback){
		if(actionName){ //Expected string or object
			...
			if(typeof actionNamesList === 'object'){
				...
				if(!context.channel || !actionNamesList[context.channel]){
					throw new Error('Cannot execute action: no action for channel ' + context.channel);
				}
				actionName = actionNamesList[context.channel];
			}
