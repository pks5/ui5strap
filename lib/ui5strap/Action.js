...
(function ui5strapAction(){
	...
	*/
	Action.blessController = function(controllerImpl){
		...
		controllerImpl.onInit = function(oEvent){ 
			...
			if(viewData && "app" in viewData){
				...
				if(appEvents && "controller" in appEvents){
					...
					//Controller pageShow events
					if("pageShow" in controllerEvents){
						var updateEvents = controllerEvents.pageShow,
							updateEventsLength = updateEvents.length;

						for(var i = 0; i < updateEventsLength; i++){
							viewControllerEvents.pageShow.push(updateEvents[i]);
						}
				...
				if(viewContent.length > 0){
					...
					if(typeof(viewEvents) !== 'undefined' && null !== viewEvents){
						...
						if(typeof viewEvents === 'object'){
							...
							if('pageShow' in viewEvents){
								viewControllerEvents.pageShow.push(viewEvents.pageShow);
							}
		...
		var oldOnPageShow = 'onPageShow' in controllerImpl ? controllerImpl.onPageShow : null;

		/*
		* @param uEvent The update event fired by SappFrame. PLEASE NOTE: uEvent is NOT a standard SAPUI5 event!
		*/
		controllerImpl.onPageShow = function(customEvent){ 
			var controller = this,
				_this = this,
				view = this.getView(),
				viewId = view.getId(),
			...
			if("parameters" in viewData.options){
				uEvent.parameters = viewData.options.parameters;
			}
			...
			//Finally, run all the pageShow events
			var viewCustomData = view.sappData;

			var updateEvents = viewCustomData[actionEventsParam].controller.pageShow,
				updateEventsLength = updateEvents.length;

			for(var i = 0; i < updateEventsLength; i++){
			 	var actionGroupId = updateEvents[i];
				viewData.app.log.debug("Executing action '" + actionGroupId + "' (view: '" + viewId + "', event: 'onPageShow') ...");
				ui5strap.Action.run({
					"parameters" : actionGroupId, 
					"uEvent" : uEvent,
					"controller" : controller,
			...
			if(null !== oldOnPageShow){
				oldOnPageShow.call(this, uEvent);
			}
		};
