...
(function(){
	...
	//----------------- STATIC methods -------------------
	ViewerMulti.UI5OS_CONTAINER_ID = "ui5strap-body";
	...
	*/
	ViewerMultiProto.showSapplication = function(sappId, transitionName, callback){ 
		...
		if(!appInstance.domRef){
			...
			appContainer.id = appInstance.getDomId();
			
			var appContent = document.createElement('div');
			appContent.className = 'ui5strap-app-content';
			appContent.id = appInstance.getDomId() + '---content';
			appContainer.appendChild(appContent);

			var appOverlay = document.createElement('div');
			appOverlay.className = 'ui5strap-app-overlay ui5strap-hidden';
			appOverlay.id = appInstance.getDomId() + '---overlay';
			appContainer.appendChild(appOverlay);

			var appLoader = document.createElement('div');
			appLoader.className = 'ui5strap-app-loader ui5strap-hidden';
			appLoader.id = appInstance.getDomId() + '---loader';
			appContainer.appendChild(appLoader);

			var appSplash = document.createElement('div');
			appSplash.className = 'ui5strap-app-splash ui5strap-hidden';
			appSplash.id = appInstance.getDomId() + '---splash';
			appContainer.appendChild(appSplash);

			appInstance.domRef = appContainer;
			appInstance.contentDomRef = appContent;
			needAttach = true;
		}
		else{
			appInstance.domRef.className = 'ui5strap-app ui5strap-app-prepared ui5strap-hidden';
		}

		var viewer = this;
		var $currentRoot = jQuery(null === previousSapplication ?  '#ui5strap-app-initial' : previousSapplication.domRef);
		...
		var currentRootCallback = function(){
			...
			if(null !== previousSapplication){
				...
				previousSapplication.domRef.className = 'ui5strap-app ui5strap-app-inactive ui5strap-hidden';
		...
		var preparedRootCallback = function(){
			requestAnimationFrame(function(){
				appInstance.domRef.className = 'ui5strap-app ui5strap-app-current';
	...
	*/
	ViewerMultiProto.showLoader = function(callback){
		this.setLayerVisible('loader', true, callback);
	};
	...
	*/
	ViewerMultiProto.hideLoader = function(callback){
		this.setLayerVisible('loader', false, callback);
	};
	...
	* @protected
	*/
	ViewerMultiProto._initBrowser = function(){
		this._browser = {};
	...
	*/
	ViewerMultiProto._initDom = function(){
		...
		this._dom.$root = jQuery('#' + ViewerMulti.UI5OS_CONTAINER_ID);
	...
	*/  
	ViewerMultiProto.requestFullscreen =  function(element){
