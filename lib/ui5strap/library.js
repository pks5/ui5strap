...
(function(){
  ...
  ui5strap.Utils.parseIContent = function(iContent){
      ...
      if(iContentType === 'string'){
          if(jQuerySap.startsWith(iContent, "@JSON")){
            var jsonString = iContent.slice(5).replace('(', '{').replace(')', '}').replace(/'/g, '"');
            
              return JSON.parse(jsonString);
          }
          if(jQuerySap.startsWith(iContent, "@J")){
              return JSON.parse(iContent.slice(2));
          }
      }
  ...
  */
  ...
  ui5strap.require = function(modules, callback){
    ...
    for(var i = 0; i < modules.length; i++){
      ...
      if( !(scriptUrl in _requiredModules) || typeof jQuerySap.getObject(modules[i]) === 'undefined'){
