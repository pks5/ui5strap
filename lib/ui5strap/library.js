...
(function(){
  ...
  * @constructor
  */
  ui5strap.Transition = function(transitionName, $currentRoot, $preparedRoot, transitionId){
    this._$currentRoot = $currentRoot;
    this._$preparedRoot = $preparedRoot;
    this._transitionId = transitionId;

    var transitionClass = transitionName;

    this.prepare = function (){
      if(null !== this._$currentRoot){
          this._$currentRoot.addClass(transitionClass + ' ' + transitionClass+'-current');
      }

      if(null !== this._$preparedRoot){
          this._$preparedRoot.addClass(transitionClass + ' ' + transitionClass+'-next').removeClass('ui5strap-hidden');
      }
    };

    this.execute = function (currentRootCallback, preparedRootCallback){
      jQuery.sap.log.debug("executing transition");
      var transitionEndEvent = ui5strap.support.transitionEndEvent;

      if(transitionEndEvent){
        if(currentRootCallback && null !== this._$currentRoot){   
          this._$currentRoot
          //  .off(transitionEndEvent)
          .one(transitionEndEvent, currentRootCallback);
        }
        if(preparedRootCallback && null !== this._$preparedRoot){   
          this._$preparedRoot
          //.off(transitionEndEvent)
          .one(transitionEndEvent, preparedRootCallback);
        }
      }

      if(transitionName && transitionEndEvent){ 
        jQuery.sap.log.debug('[TRANSITION] ' + this._transitionId + ' : starting transition ' + transitionName);
          if(null !== this._$currentRoot){
            this._$currentRoot.addClass(transitionClass+'-current-out');
          }
                  
          if(null !== this._$preparedRoot){     
            this._$preparedRoot.removeClass(transitionClass + '-next');
          }
      }
      else{
        jQuery.sap.log.debug('[TRANSITION] ' + this._transitionId + ' : no transition');
        this._$preparedRoot && this._$preparedRoot.removeClass('ui5strap-hidden');

        requestAnimationFrame(function(){
          currentRootCallback && currentRootCallback();
          preparedRootCallback && preparedRootCallback();
        });
      }
  ...
  */
  ...
  ui5strap.layer.loaderVisible = false;
  ui5strap.layer.$loader = jQuery('#ui5strap-loader');

  ui5strap.layer.splashVisible = false;
  ui5strap.layer.$splash = jQuery('#ui5strap-splash');
  /*
  * @Private
  */
  ui5strap.layer.setLayerVisible = function(layerName, visible, callback, option){
      var $layer = this['$' + layerName],
          statusVarName = layerName + 'Visible';


      if(visible && ui5strap.layer[statusVarName] || $layer.length === 0){
        callback && callback();
        return;
      }

      this[statusVarName] = visible;
  ...
  */
  ...
  var _checkModules = function(_this){
    jQuerySap.log.debug('ui5strap::_checkModules');
    var i = 0;
    while(i < _callbackStack.length){
      ...
      if(true === modulesExecuted){
        jQuerySap.log.debug('ui5strap::_checkModules: callback called.');

        var callee = _callbackStack.shift();
        callee.callback();
      }
      else{
        jQuerySap.log.debug('ui5strap::_checkModules: some modules are still loading.');
        break;
      }
    }
  ...
  */
  ui5strap.require = function(modules, callback){
    ...
    jQuerySap.log.debug('ui5strap.require(' + modules.join(', ') + ')');
    ...
    var success = function anon_requireSuccess(){
      ...
      if(0 === _callbackPending){
        ...
        if(null === _callbackTimer){
          jQuerySap.log.debug('ui5strap.require: created timer.');
