...
*/
	...
	var _executeTransition = function(transList){
		transList.pageChange.transition.execute(
			function anon_transitionCurrentComplete(){
				...
				if(0 === transList.callI && typeof transList.callback === 'function'){
	...
	var _executeTransitions = function(_this){
		...
		for(var i = 0; i < pendingTransitionsLength; i++){
			callbacks.push(_this._targetTransitions[_this._pendingTransitions[i]].callback);
		}
		...
		for(var i = 0; i < pendingTransitionsLength; i++){
			transList.pageChange = _this._targetTransitions[_this._pendingTransitions[i]];
	...
	*/
	...
	NavContainerBaseProto.setAggregation = function(aggregationName, newContent, suppressInvalidate){
		this.setPage(newContent, aggregationName);
		return this;
	};
	...
	NavContainerBaseProto.setPage = function(page, target, transitionName, callback){
		...
		this.toPage(page, target, transitionName, function(){
			sap.ui.core.Control.prototype.setAggregation.call(_this, target, page, true);
			callback && callback();
		});
