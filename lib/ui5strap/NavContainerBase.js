...
(function(){
	...
	if("ontransitionend" in window){
		_transitionEndEvent = 'transitionend';
	}
	else if("onwebkittransitionend" in window){
		_transitionEndEvent = 'webkittransitionend';
	}
	else if("onotransitionend" in window){
		_transitionEndEvent = 'otransitionend';
	}
	else if("onmstransitionend" in window){
		_transitionEndEvent = 'mstransitionend';
	}
	...
	*/
	...
	NavContainerBaseProto.getTargetContent = function(target){
		var $targetContainer = this.$().find('.' + this.getId() + '--' + target),
			$currentContent = $targetContainer.children().first();
	...
	NavContainerBaseProto.onAfterRendering = function(){
		...
		for(var i = 0; i < contentsKeysLength; i++){
			...
			if(!(contentKey in this._targetTransitions)){
				_placePage(contentKey, this._contents[contentKey], false); 
			}
		}	
		...
		for(var i = 0; i < pendingTransitionsLength; i++){
			...
			if(null !== page){
				targetTransition.$next = _placePage(target, page, true); 
			}
		}
