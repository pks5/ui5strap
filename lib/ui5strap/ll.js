...
function libertyLite(_appBase){
	...
	_initConfig(function _initConfigSuccess(_configData){
		...
		*/
		var _preloadJavaScript = function(js, callback){
			...
			for(var i = 0; i < js.length; i++){
				...
				if(typeof scr === 'object'){
					jsPath =("package" in scr ? jQuery.sap.getModulePath(scr['package']) : _appBase) + "/" + scr.src;
				}
				scripts.push(jsPath);
			}
		...
		LibertyAppProto.init = function(){
			...
			_preloadJavaScript(_configData.js, function preloadJavaScriptComplete(){
				//TODO FIXME
