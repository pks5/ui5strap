...
(function(){
	...
	*/
	var _placePage = function(_this, target, page, isPrepared){
			...
			_this.incAnonymousPageId(target);
			newPage.id = _this.pageDomId(target, page);
				
			NavContainerBase.pageId++;

			var $targetContainer = jQuery('#' + _this.aggregationPagesDomId(target));
	...
	*/
	NavContainerBaseProto.init = function(){
		...
		this._anonymousPages = {};
	};

	/*
	* @Private
	*/
	NavContainerBaseProto.incAnonymousPageId = function(aggregationName){
		if(!(aggregationName in this._anonymousPages)){
			this._anonymousPages[aggregationName] = 0;
		}
		
		this._anonymousPages[aggregationName]++;
	};

	/*
	* @Private
	*/
	NavContainerBaseProto.pageDomId = function(aggregationName, aggregationContent){
		if(aggregationContent === null){
			return 'navcontainer-page-' + this._anonymousPages[aggregationName];
		}
	...
	* @Private
	*/
	NavContainerBaseProto.aggregationPagesDomId = function(aggregationName){
		return 'navcontainer-pages-' + aggregationName + '---' + this.getId();
	};

	/*
	* @Private
	*/
	NavContainerBaseProto.aggregationLayersDomId = function(aggregationName){
		return 'navcontainer-layers-' + aggregationName + '---' + this.getId();
	};
	...
	*/
	NavContainerBaseProto.setAggregation = function(aggregationName, newContent, suppressInvalidate){ 
		...
		//console.log(aggregationName, newContent);
		return this;
	};
	...
	*/
	NavContainerBaseProto.toPage = function(page, target, transitionName, callback){
		...
		var $aggregation = jQuery('#' + this.aggregationPagesDomId(target));
		var $currentPage = jQuery('#' + this.pageDomId(target, currentPage));
