...
de_pksoftware.ui5strap.controls.NavRenderer.render = function(rm, oControl) {
	...
	rm.addClass('nav');
	if(de_pksoftware.ui5strap.NavType.Default !== type){
		rm.addClass(de_pksoftware.ui5strap.BSNavType[type]);
	}
	...
	if(de_pksoftware.ui5strap.NavBarAlignment.None !== navbarAlign){
		rm.addClass('navbar-nav');
		if(de_pksoftware.ui5strap.NavBarAlignment.Default !== navbarAlign){
			rm.addClass(de_pksoftware.ui5strap.BSNavBarAlignment[navbarAlign]);
		}
	}
	rm.writeClasses();
	rm.write(">");
