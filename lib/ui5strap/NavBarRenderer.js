...
(function(){
	...
	ui5strap.NavBarRenderer.render = function(rm, oControl) {
		...
		if(null !== toggle){
			if(toggle.getBsAction() !== ui5strap.BsAction.ToggleNavBar || toggle.getType() !== ui5strap.ButtonType.Default){
				throw new Error('NavBar toggle button must be of type default and set to bsAction ToggleNavBar!');
			}
			rm.renderControl(toggle);
		}
