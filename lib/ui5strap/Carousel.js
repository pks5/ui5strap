...
(function(){
  ...
  CarouselProto.init = function(){
    ...
    this.$inner = _findPart(this, 'inner');
  ...
  CarouselProto.setIndex = function(newIndex){
    ...
    if(!this.getDomRef()){ 
      ...
      var innerClass = "carousel-inner carousel-current-" + newIndex;
      if(newIndex === 0){
        innerClass += " carousel-current-first";
      }
      if(newIndex === this.items.length-1){
        innerClass += " carousel-current-last";
      }
      //Refresh viewport class
      this.$inner.attr("class", innerClass);
      //Move the lane
