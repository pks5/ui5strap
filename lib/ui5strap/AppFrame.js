...
(function(){
	...
	*/
	...
	AppFrameProto.validatePage = function(data){
		...
		viewData.viewData.config = viewData; //should be the original data from config ???
	 ...
	 * Create a new page
	 */
	AppFrameProto.createPage = function(viewData){
		return _addPageToCache(this, viewData);
	};
	...
	AppFrameProto.toPage = function (viewData, callback) {
		...
		var oPage = _addPageToCache(this, viewData);

		if(viewData.vTarget){
			this.vTargets[viewData.target] = oPage;
		
			return;
		}

		//Set target busy
		jQuery.sap.log.debug('[FR][' + viewData.target + '] now busy');
		...
		this.getNavContainer().toPage(
			...
			function(){
				...
				jQuery.sap.log.debug('[FR][' + viewData.target + "] available");
