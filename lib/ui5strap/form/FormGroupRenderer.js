	...
	de_pksoftware.ui5strap.form.FormGroupRenderer.render = function(rm, oControl) {
		...
		if('' !== label){
			...
			if(oControl.getParent().getType() === 'horizontal'){
				var labelColumn = oControl.getLabelColumn();
				if(null === labelColumn){
					throw new Error('You need to define a label column if you use horizontal forms.');
				}

				var columsMedium = labelColumn.getColumnsMedium(),
					columsLarge = labelColumn.getColumnsLarge(),
					columsSmall = labelColumn.getColumnsSmall(),
					columsExtraSmall = labelColumn.getColumnsExtraSmall();
