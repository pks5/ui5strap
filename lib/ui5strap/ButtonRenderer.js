	...
	ui5strap.ButtonRenderer.startRender = function(rm, oControl, options) {
		...
		//Navbar toggle
			...
			if(!(parent instanceof ui5strap.NavBar)){
				throw new Error('NavBar toggle button is only allowed inside a ui5strap.NavBar control.');
				return;
			}
			rm.write(' type="button" class="navbar-toggle" data-toggle="collapse" data-target="#' + parent.getCollapseId() + '">');
			return;
		}
