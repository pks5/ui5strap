...
(function(){
	...
	jQuerySap.declare('ui5strap.ViewerSingle');

	jQuerySap.require('ui5strap.library');

	jQuerySap.require('ui5strap.ViewerBase');

	ui5strap.ViewerBase.extend('ui5strap.ViewerSingle');
	...
	var _showApp = function(_this, appInstance){
		...
		appInstance.getRootControl().placeAt(appInstance.config.options.container);

		appInstance.show(function(){
			appInstance.setLoaderVisible(false, function setLoaderVisible_complete(){
				appInstance.setSplashVisible(false, function setSplashVisible_complete(){
					appInstance.shown();
				});
			});
		});
		...
		appConfig.load(ui5strap.AppConfig.processOption("app", _this.options.app), function load_complete(configDataJSON){
			...
			ui5strap.require(configAppNode.module, function require_complete(){
				var AppConstructor = jQuerySap.getObject(configAppNode.module),
					appInstance = new AppConstructor(appConfig, _this);
	...
	var ViewerSingleProto = ui5strap.ViewerSingle.prototype;

	ViewerSingleProto.init = function(){
	...
	* @Override
	*/
	ViewerSingleProto.start = function(callback){
		var _this = this;
	...
	* @deprecated
	*/
	ViewerSingleProto.ready = ViewerSingleProto.start;
}());