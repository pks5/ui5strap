	...
	*/
	AppLiteProto.init = function(_configData){
		...
		this.config.getEvents = function(eventGroup, eventName, viewName){
			...
			if(_configData.a_events 
				&& _configData.a_events[eventGroup] 
				&& _configData.a_events[eventGroup][eventName]){
				eventList = eventList.concat(_configData.a_events[eventGroup][eventName]);
			}
			console.log(eventGroup, eventName, viewName)
			if(viewName){
				var viewData = this.getViewData(viewName); console.log(viewName, viewData);
				if(viewData.a_events 
					&& viewData.a_events[eventGroup] 
					&& viewData.a_events[eventGroup][eventName]){
					
					eventList = eventList.concat(viewData.a_events[eventGroup][eventName]);
		...
		_preloadCss(_configData.css, appBase, function preloadCssComplete(){
			...
			_preloadJavaScript(_configData.js, appBase, function preloadJavaScriptComplete(){
				_preloadViews(views, function preloadViewsComplete(){
					_start(_this);
				});
			});
		});
