(function a__Action(){
	...
	jQuerySap.declare('a__.Action');

	jQuerySap.require("a__.ActionContext");
	jQuerySap.require('a__.ActionModule');

	sap.ui.base.Object.extend("a__.Action");

	var Action = a__.Action,
		ActionModule = a__.ActionModule;
	...
	*/
	Action.run = function(action){
		...
		var context = new a__.ActionContext(action);
	...
	*/
	Action.blessController = function(controllerImpl){
		...
		controllerImpl.a__run = function(oEvent){
		...
		* @Deprecated
		*/
		controllerImpl.runAction = controllerImpl.a__run;

		var actionEventsParam = a__.ActionContext.createActionParam(ActionModule.PARAM_ACTION_EVENTS);
		...
		controllerImpl.onInit = function(oEvent){ 
			...
			viewControllerEvents = viewCustomData["a__events"].controller;
			...
			if(viewData && "app" in viewData){
				...
				if("options" in viewData){
					...
					if( 
						...
						( "a__events" in appConfigData.views[viewName] ) && 
						( "controller" in appConfigData.views[viewName]["a__events"] ) ){
						
						var controllerEvents = appConfigData.views[viewName]["a__events"].controller;
				...
				if(viewContent.length > 0){
					...
					if(typeof(viewEvents) !== 'undefined' && null !== viewEvents){
						...
						if(typeof viewEvents === 'object'){
							if('controller' in viewEvents){
								var controllerEvents = viewEvents.controller;
								if('init' in controllerEvents){
									var initEvents = controllerEvents.init,
										initEventsLength = initEvents.length;
		...
		*/
		controllerImpl.onUpdateView = function(uEvent){ 
			...
			var updateEvents = viewCustomData["a__events"].controller.updateView,
				updateEventsLength = updateEvents.length;
