...
(function(){
	...
	sap.ui.core.Control.extend("net__rozzo__games.quintus__app.controls.Quintus", {
		metadata : {
			...
			// ---- control specific ----
			library : "net__rozzo__games.quintus__app",
	...
	QuintusProto.onAfterRendering = function(){
		var canvasId = this.getId()+ '---canvas',	
			appPath = jQuery.sap.getModulePath(this.getMetadata().getLibraryName());
		//--------------------
		...
		var Q = window.Q = Quintus(
				{
					...
					imagePath: appPath + "/data/images/",
                  		audioPath: appPath + "/data/audio/",
                  		dataPath: appPath + "/data/data/"
				}
				)
