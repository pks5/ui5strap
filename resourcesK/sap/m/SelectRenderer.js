...
sap.m.SelectRenderer._renderSelectElement=function(r,s,S){var n=s.getName();r.write('<select class="'+sap.m.SelectRenderer.CSS_CLASS+"Native"+'"');if(n){r.writeAttributeEscaped("name",n)}r.writeAttribute("tabindex","-1");r.write(">");this._renderOptions(r,s,S);r.write("</select>")};
sap.m.SelectRenderer._renderOptions=function(r,s,S){var I=s.getItems(),a=I.length,i=0;for(;i<a;i++){r.write("<option>");r.writeEscaped(I[i].getText());r.write("</option>")}if(a===0){r.write("<option>"+S+"</option>")}};
