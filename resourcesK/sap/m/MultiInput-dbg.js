...
sap.m.MultiInput.prototype.init = function() {
	...
	this._tokenizer.attachTokenChange(function(args) {
		...
		var addedTokens = args.getParameter("tokensAdded");
		var token = args.getParameter("token");
		if ((addedTokens && addedTokens.length > 0) || token) {

			that._scrollToEnd();

			// need to have bScrollToEnd pending since it could happen that rendering did not yet take place
			that._bScrollToEnd = true;
		}
	});
...
sap.m.MultiInput.prototype.exit = function() {
	if (this._sResizeHandlerId) {
		...
		delete this._sScrollerResizeListenerId;
	}
}
...
sap.m.MultiInput.prototype._setContainerSizes = function() {
	...
	var shadowDiv = jMultiInput.children(".sapMMultiInputShadowDiv")[0];
	jQuery(shadowDiv).text(this.getValue());
	var inputWidth = jQuery(shadowDiv).outerWidth();
	var inputContainer = jMultiInput.find(".sapMMultiInputInputContainer")[0];
	var scrollDiv = jMultiInput.find(".sapMMultiInputScrollContainer")[0];

	var tokenizerWidth = this._tokenizer.$().outerWidth(true);
	var totalWidth = jQuery(scrollDiv).width();

	var innerWidth = tokenizerWidth + inputWidth;

	if (innerWidth < totalWidth) {
		inputWidth = totalWidth - tokenizerWidth;
	}

	jQuery(jQuery(inputContainer).find(".sapMInputBaseInner")[0]).css("width", inputWidth + "px");
...
sap.m.MultiInput.prototype.onAfterRendering = function() {
	...
	if (this._bScrollToEnd) {
		// initialize this._bScrollToEnd
...
sap.m.MultiInput.prototype._scrollBy = function(iOffset) {
	var thisDomRef = this.getDomRef();
	if (!thisDomRef) {
		return;
	}
	...
	var scrollDiv = jMultiInput.find(".sapMMultiInputScrollContainer")[0];
	var scrollOffset = scrollDiv.scrollLeft + iOffset;
	if (scrollOffset < 0) {
		scrollOffset = 0;
	}
	scrollDiv.scrollLeft = scrollOffset;
};

sap.m.MultiInput.prototype._scrollToEnd = function(bInitialize) {
	var thisDomRef = this.getDomRef();
	if (!thisDomRef) {
		return;
	}
	...
	var jMultiInput = jQuery(thisDomRef);
	var scrollDiv = jMultiInput.find(".sapMMultiInputScrollContainer")[0];
	var scrollOffset = scrollDiv.scrollWidth - jQuery(scrollDiv).width();
	if (scrollOffset < 0) {
		scrollOffset = 0;
	}
